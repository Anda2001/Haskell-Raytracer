...
vecParser = pMap3 vec3 x y z
  where
    x = doubleParser `andThen` (ws `andThen` char ',' `andThen` ws)
    y = doubleParser `andThen` (ws `andThen` char ',' `andThen` ws)
    z = doubleParser `andThen` ws
    vec3 (x, _) (y, _) (z, _) = Vec3 x y z

vec3 :: Char -> Char -> Double -> Vec3
vec3 = error "not implemented"
...
materialParser = error "implement materialParser"